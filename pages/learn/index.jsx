...
const LearnListPage = ({ learnListData }) => {
    ...
    const [content, setContent] = useState(learnListData)
   

    const getAllTags = useMemo(() => {
        learnListData.map((post) => (
            post.tags.map((tag) => (
                tagList.push(tag)
            ))
        ))
        tagCntList['All'] = learnListData.length
        tagList.map((tag) => (
            !tagCntList[tag]
            ? tagCntList[tag] = 1
            : tagCntList[tag] += 1
                        ...
                        <ul className='post-ul'>
                            {
                                content.map(
                                    (data) => (
                                        <li key = {data.id} className = 'post-li'>
                                            <Link href={`/learn/${data.id}`} >
                                                <a>
                                                    {data.title}
                                                </a>
                                            </Link>
...
export const getServerSideProps = async () => {
    ...
    const res = await fetch(`${API_HOST}/learn`);
    const learnListData = await res.json();
    learnListData.sort( (a, b) => {
        if (a.createdAt < b.createdAt) return 1;
        if (a.createdAt > b.createdAt) return -1;
    })
